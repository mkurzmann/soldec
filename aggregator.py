...
def aggregate_instruction(begin, block, live):
    ...
    for i in indizes:
        if is_valid_at(expressions, begin, i):
            target = target_expression.writes[0]
            count = expressions[i].get_read_count(target)
            if count == 1:
                expressions[i].set_dependency(target, target_expression)
                block.set_pass_expression(begin)
...
class Aggregator(Optimizer):
    ...
    def __convert_function(self, func):
        ...
        for block in func.graph:
            ...
            while change:
                ...
                self.__aggregate_instructions(block, live)
    ...
    def __convert_instruction(instruction):
        ...
        if opcode == "STOP":
            return StopExpression(opcode, reads, writes, address)
        elif opcode == "RETURN":
            return ReturnExpression(opcode, reads, writes, address)
        elif opcode == "MOVE":
        ...
        elif isinstance(instruction, SHA3Operation):
            return SHA3Expression(opcode, reads, writes, address)
        elif opcode in log_ops:
            return LogExpression(opcode, reads, writes, address)
    ...
    @staticmethod
    def __aggregate_instructions(block, live):
        expressions = block.get_items()
        ...
        for i, expression in enumerate(expressions):
            ...
            aggregate_instruction(i, block, live)
