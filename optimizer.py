...
# rewriting of the sha3 expression - memory actions before a
# sha3 expression are encapsulated to the sha3 expression
...
# rewriting of the return expression - memory actions with free memory pointer
# before a return expression are encapsulated to the return expression
        ...
        if instruction.opcode == "RETURN":
            ...
            if begin == "$m":
                ...
                if item:
                    ...
                    block.set_nop_instruction(i)
                    tmp_list = [value, end]
                    operation = Instruction("RETURN", tmp_list, instruction.writes, instruction.address)
                    block.set_instruction(index, operation)
        local_memory.add_mapping_for_return_and_log(index, instruction)


# rewriting of log expressions - memory actions with free memory pointer
# before a log expression are encapsulated to the log expression
                ...
                if item:
                    ...
                    if len(instruction.reads) > 2:
                        for r in instruction.reads[2:]:
                            tmp_list.append(r)
                    operation = Instruction(instruction.opcode, tmp_list, instruction.writes, instruction.address)
                    block.set_instruction(index, operation)
        local_memory.add_mapping_for_return_and_log(index, instruction)
